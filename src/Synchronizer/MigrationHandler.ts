...
import { fetchSyncInfo, SyncInfo, uploadSyncInfo } from "./syncInfoUtils";
...
export default class MigrationHandler extends BaseService {
    ...
    if (remoteInfo.version) {
      if (remoteInfo.version > supportedSyncTargetVersion) {
        throw new JoplinError(
          sprintf(
            "Sync version of the target (%d) is greater than the version supported by the app (%d). Please upgrade your app.",
            remoteInfo.version,
            supportedSyncTargetVersion
          ),
          "outdatedClient"
        );
        ...
        throw new JoplinError(
          sprintf(
            "Sync version of the target (%d) is lower than the version supported by the app (%d). Please upgrade the sync target.",
            remoteInfo.version,
            supportedSyncTargetVersion
          ),
          "outdatedSyncTarget"
  ...
  public async initializeSyncTarget(targetVersion = 0) {
    const supportedSyncTargetVersion = parameters_.syncVersion;
    const syncTargetInfo = { version: 0 };

    if (syncTargetInfo.version > supportedSyncTargetVersion) {
      throw new JoplinError(
        sprintf(
          "Sync version of the target (%d) is greater than the version supported by the app (%d). Please upgrade your app.",
          syncTargetInfo.version,
          supportedSyncTargetVersion
        ),
        "outdatedClient"
      );
    ...
    if (syncTargetInfo.version === 0 || syncTargetInfo.version === 1) {
      this.logger().info(
        'MigrationHandler: Sync target version is 0 or 1 - creating "locks" and "temp" directory:',
        syncTargetInfo
      );
    ...
    try {
      for (
        let newVersion = syncTargetInfo.version + 1;
        ...
        this.logger().info(
          `MigrationHandler: Migrating from version ${fromVersion} to version ${newVersion}`
        );
        ...
        try {
          ...
          this.logger().info(
            `MigrationHandler: Done migrating from version ${fromVersion} to version ${newVersion}`
          );
        } catch (error) {
