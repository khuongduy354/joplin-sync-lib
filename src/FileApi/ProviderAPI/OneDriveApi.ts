...
export class OneDriveApi {
  ...
  public async execTokenRequest(code: string, redirectUri: string) {
    ...
    const r = await fetch(this.tokenBaseUrl(), {
      method: "POST",
      body: helper.objectToQueryString(body),
      headers: {
  ...
  public async uploadChunk(
    ...
    if (buffer) {
      ...
      const chunk = await singleton
        .fsDriver()
        .readFileChunk(handle, options.contentLength);
      const buffer = Buffer.from(chunk as string, "base64");
      options.body = buffer;
    }
    ...
    const response = await singleton.fetch(url, options);
    return response;
  }

  public async uploadBigFile(url: string, options: any) {
    const response = await singleton.fetch(url, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      if (options.body) {
        ...
        handle = await singleton.fsDriver().open(options.path, "r");
      }
      ...
      try {
        for (let i = 0; i < numberOfChunks; i++) {
          ...
          const response = await this.uploadChunk(uploadUrl, handle, buffer, {
            startByte: startByte,
        ...
        if (handle) await singleton.fsDriver().close(handle);
      }
    }
  }
  ...
  public async exec(
    ...
    if (query) {
      url += url.indexOf("?") < 0 ? "?" : "&";
      url += helper.objectToQueryString(query);
    }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      try {
        if (path.includes("/createUploadSession")) {
          ...
          response = await singleton.uploadBlob(url, options);
        } else if (options.target === "string") {
          response = await singleton.fetch(url, options);
        } else {
          // file
      ...
      if (!response.ok) {
        ...
        if (
          ...
          error &&
          (error.code === "resourceModified" ||
            (error.error && error.error.code === "resourceModified"))
        ) {
  ...
  public async refreshAccessToken() {
    ...
    const response = await singleton.fetch(this.tokenBaseUrl(), {
      method: "POST",
      body: helper.objectToQueryString(body),
      headers: {
        ["Content-Type"]: "application/x-www-form-urlencoded",
