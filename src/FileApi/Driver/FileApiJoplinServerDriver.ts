...
export default class FileApiDriverJoplinServer {
  ...
  public async list(path: string, options: any = null) {
    ...
    const query = options.context?.cursor
      ? { cursor: options.context.cursor }
      : null;

    const results = await this.api().exec(
      "GET",
      `${this.apiFilePath_(searchPath)}/children`,
      query
    );
  ...
  public async get(path: string, options: any) {
    ...
    try {
      const response = await this.api().exec(
        "GET",
        `${this.apiFilePath_(path)}/content`,
        null,
        null,
        null,
        options
  ...
  public async put(path: string, content: any, options: any = null) {
    try {
      const output = await this.api().exec(
        "PUT",
        `${this.apiFilePath_(path)}/content`,
        options && options.shareId ? { share_id: options.shareId } : null,
        content,
        {
  ...
  public async releaseLock(
    ...
    await this.api().exec(
      "DELETE",
      `api/locks/${type}_${clientType}_${clientId}`
    );
  }
