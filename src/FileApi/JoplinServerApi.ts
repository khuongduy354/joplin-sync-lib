...
export default class JoplinServerApi {
  ...
  //   return personalizedUserContentBaseUrl(
  //     userId,
  //     this.baseUrl(),
  //     this.options_.userContentBaseUrl()
  //   );
  // }
  ...
  private hidePasswords(o: any): any {
    if (typeof o === "string") {
      try {
        ...
        if (!output) return o;
        if (output.password && !this.debugRequestsShowPasswords_)
          output.password = "******";
      ...
      if (output.password && !this.debugRequestsShowPasswords_)
        output.password = "******";
      if (output["X-API-AUTH"] && !this.debugRequestsShowPasswords_)
        output["X-API-AUTH"] = "******";
      return output;
    }
  }
    ...
    if (options.body) {
      const serialized =
        typeof options.body !== "string"
          ? JSON.stringify(this.hidePasswords(options.body))
          : this.hidePasswords(options.body);
  ...
  public async exec(
    ...
    for (let i = 0; i < 2; i++) {
      try {
        const response = await this.exec_(
          method,
          path,
          query,
          body,
