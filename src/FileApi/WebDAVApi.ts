...
export default class WebDavApi {
  ...
  private lastRequests_: RequestLog[];
  ...
  public logRequest_(request: any, responseText: string): void {
    if (this.lastRequests_.length > 10) this.lastRequests_.splice(0, 1);

    const serializeRequest = (r) => {
      const options = { ...r.options };
      if (typeof options.body === "string")
        options.body = options.body.substr(0, 4096);
      const output = [];
  ...
  public lastRequests(): RequestLog[] {
  ...
  public valueFromJson(
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (
        typeof key === "string" &&
        (typeof output !== "object" || !(key in output))
      )
        return null;
      output = output[key];
    }
  ...
  public stringFromJson(json: any, keys: (string | number)[]): string | null {
    return this.valueFromJson(json, keys, "string");
  }

  public objectFromJson(json: any, keys: (string | number)[]): any {
    return this.valueFromJson(json, keys, "object");
  }

  public arrayFromJson(json: any, keys: (string | number)[]): any[] | null {
  ...
  private requestToCurl_(url: string, options: any): string {
    const output = [];
    output.push("curl");
    output.push("-v");
  ...
  private handleNginxHack_(
    ...
    const responseArray = this.arrayFromJson(jsonResponse, [
      "d:multistatus",
      "d:response",
    ]);
    if (responseArray && responseArray.length === 1) {
      const propStats = this.arrayFromJson(jsonResponse, [
        "d:multistatus",
        "d:response",
        0,
        "d:propstat",
      ...
      let count404 = 0;
      for (let i = 0; i < propStats.length; i++) {
        const status = this.arrayFromJson(jsonResponse, [
          "d:multistatus",
          "d:response",
          0,
          "d:propstat",
        ...
        if (status && status.length && status[0].indexOf("404") >= 0)
          count404++;
      }
  ...
  public async exec(
    ...
    if (options.source === "file" && (method === "POST" || method === "PUT")) {
      if (fetchOptions.path) {
        const fileStat = await singleton.fsDriver().stat(fetchOptions.path);
        if (fileStat)
    ...
    const loadResponseJson = async () => {
      ...
      if (!responseJson_)
        throw newError("Cannot parse XML response", response.status);
    ...
    if (
      ...
      !("d:multistatus" in output)
    )
      throw newError("Not a valid WebDAV response");
