...
function setSyncTargetName(name: string) {
  ...
  sleepTime =
    syncTargetId_ === SyncTargetRegistry.nameToId("filesystem") ? 1001 : 100; // 400;
  isNetworkSyncTarget_ = [
    "nextcloud",
    "dropbox",
    "onedrive",
    "amazon_s3",
...
const createNoteAndResource = async (
  options: CreateNoteAndResourceOptions = null
) => {
  options = {
...
async function setupDatabaseAndSynchronizer(id: number, options: any = null) {
  ...
  if (!synchronizers_[id]) {
    ...
    const syncTarget = new FileSystemSyncTarget(db(id));
    await initFileApi();
...
function mustRunInBand() {
  if (!process.argv.includes("--runInBand")) {
    throw new Error(
      "Tests must be run sequentially for this sync target, with the --runInBand arg. eg `yarn test --runInBand`"
    );
  }
}
...
async function initFileApi() {
  ...
  if (syncTargetId_ === SyncTargetRegistry.nameToId("filesystem")) {
    ...
    const authData = fs.readFileSync(
      await credentialFile("onedrive-auth.txt"),
      "utf8"
    );
    ...
    const {
      FileApiDriverAmazonS3,
    } = require("../file-api-driver-amazon-s3.js");
    ...
    if (!amazonS3Creds || !amazonS3Creds.credentials)
      throw new Error(
        `AWS auth JSON missing in ${amazonS3CredsPath} format should be: { "credentials": { "accessKeyId": "", "secretAccessKey": "", } "bucket": "mybucket", region: "", forcePathStyle: ""}`
      );
    const api = new S3Client({
      region: amazonS3Creds.region,
      credentials: amazonS3Creds.credentials,
      s3UseArnRegion: true,
      forcePathStyle: amazonS3Creds.forcePathStyle,
      endpoint: amazonS3Creds.endpoint,
    });
    fileApi = new FileApi(
      "",
      new FileApiDriverAmazonS3(api, amazonS3Creds.bucket)
    );
  } else if (
    syncTargetId_ === SyncTargetRegistry.nameToId("joplinServer") ||
    syncTargetId_ === SyncTargetRegistry.nameToId("joplinCloud")
  ) {
    mustRunInBand();

    const joplinServerAuth = JSON.parse(
      await readCredentialFile("joplin-server-test-units-2.json")
    );
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
async function createNTestNotes(
  n: number,
  folder: any,
  tagIds: string[] = null,
  title = "note"
...
function tempFilePath(ext: string) {
  return `${Setting.value("tempDir")}/${md5(
...
export async function naughtyStrings() {
  ...
  const t = await fs.readFile(
    `${supportDir}/big-list-of-naughty-strings.txt`,
    "utf8"
  );
  const lines = t.split("\n");
  naughtyStrings_ = [];
...
export {
  supportDir,
  createNoteAndResource,
  createTempFile,
  createTestShareData,
  simulateReadOnlyShareEnv,
  waitForFolderCount,
  afterAllCleanUp,
  exportDir,
  synchronizerStart,
