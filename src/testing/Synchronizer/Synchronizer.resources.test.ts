...
import { singleton } from "../../singleton"; // TODO: handle singleton first
...
describe("Synchronizer.resources", () => {
  ...
  it("should sync resources", async () => {
    ...
    await singleton.attachFileToNote(note1, `${supportDir}/photo.jpg`);
  ...
  it("should delete resources", async () => {
    ...
    expect(await singleton.fsDriver().exists(resourcePath1)).toBe(true);
    await synchronizerStart();
    allResources = await Resource.all();
    expect(allResources.length).toBe(0);
    expect(await singleton.fsDriver().exists(resourcePath1)).toBe(false);
  });
  ...
  it("should sync resource blob changes", async () => {
    ...
    await singleton.fsDriver().writeFile(tempFile, "1234", "utf8");
    const folder1 = await Folder.save({ title: "folder1" });
    const note1 = await Note.save({ title: "ma note", parent_id: folder1.id });
    await singleton.attachFileToNote(note1, tempFile);
    await synchronizerStart();
    ...
    await singleton.fsDriver().writeFile(modFile, "1234 MOD", "utf8");
    ...
    expect(await Resource.resourceBlobContent(resource1_1.id, "utf8")).toBe(
      "1234 MOD"
    );
  });
  ...
  it("should handle resource conflicts", async () => {
...

    {
      ...
      await singleton.fsDriver().writeFile(modFile2, "1234 MOD 2", "utf8");
...

    {
      ...
      await singleton.fsDriver().writeFile(modFile1, "1234 MOD 1", "utf8");
      ...
      await checkThrowAsync(
        async () => await Resource.resourceBlobContent(resource.id)
      );
      ...
      await resourceFetcher().start();
      await resourceFetcher().waitForAllFinished();
      const localContent = await Resource.resourceBlobContent(
        resource.id,
        "utf8"
      );
      ...
      const resourceConflictFolderId =
        await Resource.resourceConflictFolderId();
      ...
      const conflictContent = await Resource.resourceBlobContent(
        resourceIds[0],
        "utf8"
      );
      ...
      // is a top folder.
      const resourceConflictFolder = await Folder.load(
        resourceConflictFolderId
      );
  ...
  it("should not upload blob if it has not changed", async () => {
    ...
    await singleton.attachFileToNote(note, `${supportDir}/sample.txt`);
    ...
    await synchronizerStart();
    expect(await synchronizer().api().get(resourcePath)).toBe(
      "check if changed"
    );
    ...
    await Resource.updateResourceBlobContent(
      resource.id,
      `${supportDir}/sample.txt`
    );
    await synchronizerStart();
    expect(await synchronizer().api().get(resourcePath)).toBe("just testing");
    ...
    expect(await synchronizer().api().get(`${resource.id}.md`)).toContain(
      "my new title 2"
    );
  });
});
