...
class FileApi{
	...
	public async list(path = '', options: any = null): Promise<PaginatedList> {
		...
		if (options.syncItemsOnly) {
			result.items = result.items.filter((f: any) => !f.isDir && helper.isSystemPath(f.path));
		}
	...
	public async put(path: string, content: any, options: any = null) {
		...
		if (options && options.source === 'file') {
			if (!(await this.fsDriver().exists(options.path))) throw new Error(`File not found: ${options.path}`);
		}
...
async function basicDelta(path: string, getDirStatFn: Function, options: any) {
	...
	if (newContext.statsCache === null) {
		...
		newContext.statIdsCache = newContext.statsCache.filter((item: any) => helper.isSystemPath(item.path)).map((item: any) => helper.pathToId(item.path));
	...
	if (!newContext.deletedItemsProcessed) {
		...
		if (options.wipeOutFailSafe && percentDeleted >= 0.90) throw new Error("More than 90% of the notes are going to be deleted. This is likely a configuration error or bug. Sync has been aborted to prevent data loss.");
